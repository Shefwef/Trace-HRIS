...
export const ROLE_HIERARCHY: Role[] = ['SUPER_ADMIN', 'ADMIN', 'HR', 'EMPLOYEE'];
...
/** Anyone with review power over leave requests. Mirrors the old canApprove. */
export function canApproveLeave(u: { role: Role; roles?: Role[] | null }): boolean {
  return hasRole(u, 'HR') || hasRole(u, 'ADMIN') || hasRole(u, 'SUPER_ADMIN');
}
...
export function assignableRoles(actor: { role: Role; roles?: Role[] | null }): Role[] {
  ...
  if (hasRole(actor, 'HR')) return ['HR', 'EMPLOYEE'];
  return [];
}
